...
func registerHandlersInternal(api huma.API, deps HandlerDeps, handlerAppCtx handlers.ActivityAppContext, cfg *config.Config) {
	...
	handlers.RegisterS3Destinations(api, deps.S3Destination, deps.Environment)
	handlers.RegisterSystemBackups(api, deps.SystemBackup, deps.Activity, handlerAppCtx)
	handlers.RegisterJobSchedules(api, deps.JobSchedule, deps.Environment)
	handlers.RegisterVolumes(api, deps.Docker, deps.Volume, deps.Activity, deps.Environment, handlerAppCtx)
