...
func (h *ProjectHandler) RedeployProject(ctx context.Context, input *RedeployProjectInput) (*RedeployProjectOutput, error) {
	response, err := h.runProjectActivityActionResponseInternal(ctx, input.EnvironmentID, input.ProjectID, h.redeployProjectActivityConfigInternal())
	if err != nil {
		return nil, err
	}
...
func (h *ProjectHandler) redeployProjectActivityConfigInternal() projectActivityActionConfigInternal {
	return projectActivityActionConfigInternal{
		...
		Action: func(runtimeCtx context.Context, projectID string, user models.User) error {
			return h.projectService.RedeployProject(runtimeCtx, projectID, user)
		},
