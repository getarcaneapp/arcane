...
func TestNormalizeContainerLine(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "stderr colon prefix is application content",
			raw:           "stderr:error message",
			expectedLevel: "stdout",
			expectedMsg:   "stderr:error message",
			expectedTS:    "",
		},
		{
			name:          "repeated stderr marker keeps content",
			raw:           "[STDERR] [STDERR] error occurred",
			expectedLevel: "stderr",
			expectedMsg:   "[STDERR] error occurred",
			expectedTS:    "",
		},
		{
		...
		{
			name:          "Docker timestamp before stderr marker",
			raw:           "2024-06-01T12:00:00.000Z [STDERR] late failure",
			expectedLevel: "stderr",
			expectedMsg:   "late failure",
			expectedTS:    "2024-06-01T12:00:00Z",
		},
		{
			name:          "empty line",
...
func TestNormalizeProjectLine(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "Docker timestamp before service prefix",
			raw:             "2024-06-01T12:00:00.500Z web | booted",
			expectedLevel:   "stdout",
			expectedService: "web",
			expectedMsg:     "booted",
			expectedTS:      "2024-06-01T12:00:00.5Z",
		},
		{
			name:            "stderr marker and Docker timestamp after service prefix",
			raw:             "web | [STDERR] 2024-06-01T12:00:00.250Z bind failed",
			expectedLevel:   "stderr",
			expectedService: "web",
			expectedMsg:     "bind failed",
			expectedTS:      "2024-06-01T12:00:00.25Z",
		},
		{
			name:            "application timestamp inside message is preserved",
			raw:             "2024-06-01T09:00:00Z web | retried at 09:05:00.000 again",
			expectedLevel:   "stdout",
			expectedService: "web",
			expectedMsg:     "retried at 09:05:00.000 again",
			expectedTS:      "2024-06-01T09:00:00Z",
		},
	}
