...
func GenerateManagerClientMTLSAssets(cfg *Config, envID string, envName string) (*GeneratedMTLSAssets, error) {
	if !shouldAutoGenerateManagerCAInternal(cfg) {
		return nil, nil
	}
	...
	clientCertPath, clientKeyPath, err := ensureClientCertificateInternal(assetsDir, envID, envName)
	if err != nil {
		return nil, err
	}
...
func ensureClientCertificateInternal(assetsDir string, envID string, envName string) (string, string, error) {
	...
	if fileExistsInternal(clientCertPath) && fileExistsInternal(clientKeyPath) {
		if err := validateGeneratedClientCertificateInternal(clientCertPath, clientKeyPath, expectedCommonName); err == nil {
...
func validateGeneratedClientCertificateInternal(certPath, keyPath string, expectedCommonName string) error {
	cert, err := readCertificateInternal(certPath)
	if err != nil {
		return err
	}
