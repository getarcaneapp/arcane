...
func TestGenerateManagerClientMTLSAssets(t *testing.T) {
	...
	assets, err := GenerateManagerClientMTLSAssets(cfg, "env-123", "Lab Server")
...
func TestValidateGeneratedClientCertificateInternal_RejectsMismatchedKeyPair(t *testing.T) {
	...
	clientCertPath, _, err := ensureClientCertificateInternal(assetsDir, "env-123", "Lab Server")
	require.NoError(t, err)
	...
	err = validateGeneratedClientCertificateInternal(clientCertPath, clientKeyPath, "Lab-Server-env-123")
	require.Error(t, err)
	require.Contains(t, err.Error(), "does not match private key")
}

func TestEnsureClientCertificateInternal_RegeneratesWhenCommonNameChanges(t *testing.T) {
	assetsDir := t.TempDir()
	...
	require.NoError(t, err)
	require.NotEqual(t, string(originalPEM), string(updatedPEM))
