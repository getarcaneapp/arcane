...
func TestUploadAndRestoreReturnsBadRequestWhenNoFileProvided(t *testing.T) {
	h := &VolumeHandler{volumeService: &VolumeService{}}
...
func TestParseVolumeWorkspaceManifestInternal(t *testing.T) {
	_, err := handlerutil.ParseMultipartJSONPart[volumetypes.WorkspaceUpdateManifest](multipart.Form{}, "manifest")
	require.Error(t, err)
	_, err = handlerutil.ParseMultipartJSONPart[volumetypes.WorkspaceUpdateManifest](multipart.Form{Value: map[string][]string{"manifest": {"{"}}}, "manifest")
	require.Error(t, err)
	_, err = handlerutil.ParseMultipartJSONPart[volumetypes.WorkspaceUpdateManifest](multipart.Form{Value: map[string][]string{"manifest": {"{}", "{}"}}}, "manifest")
	require.Error(t, err)

	manifest, err := handlerutil.ParseMultipartJSONPart[volumetypes.WorkspaceUpdateManifest](multipart.Form{Value: map[string][]string{
