...
func provideVolumeServiceInternal(db *database.DB, docker *services.DockerClientService, event *services.EventService, activity *services.ActivityService, settings *services.SettingsService, container *services.ContainerService, image *services.ImageService, s3Destination *services.S3DestinationService, cfg *config.Config) *services.VolumeService {
	return services.NewVolumeService(db, docker, event, activity, settings, container, image, s3Destination, cfg.BackupVolumeName, cfg.EncryptionKey)
}
