...
type Jobs struct {
	AutoUpdate             *pkgscheduler.AutoUpdateJob
	ImagePolling           *pkgscheduler.ImagePollingJob
	DockerClientRefresh    *pkgscheduler.DockerClientRefreshJob
	Analytics              *pkgscheduler.AnalyticsJob
	EventCleanup           *pkgscheduler.EventCleanupJob
	PruningVolumeHelper    *pkgscheduler.PruningVolumeHelperJob
	ExpiredSessionsCleanup *pkgscheduler.ExpiredSessionsCleanupJob
	ScheduledPrune         *pkgscheduler.ScheduledPruneJob
	FilesystemWatcher      *pkgscheduler.FilesystemWatcherJob
	VulnerabilityScan      *pkgscheduler.VulnerabilityScanJob
	AutoHeal               *pkgscheduler.AutoHealJob
}
...
func provideAnalyticsJobInternal(settings *services.SettingsService, kv *services.KVService, cfg *config.Config) *pkgscheduler.AnalyticsJob {
	return pkgscheduler.NewAnalyticsJob(settings, kv, nil, cfg)
}
...
func provideFilesystemWatcherJobInternal(ctx context.Context, project *services.ProjectService, template *services.TemplateService, settings *services.SettingsService, cfg *config.Config) *pkgscheduler.FilesystemWatcherJob {
	job, err := pkgscheduler.RegisterFilesystemWatcherJob(ctx, project, template, settings, cfg.ProjectScanMaxDepth)
	if err != nil {
