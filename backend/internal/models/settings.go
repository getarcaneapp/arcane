...
type Settings struct {
	...
	HostTerminalEnabled             SettingVariable `key:"hostTerminalEnabled,authrequired,envOverride" meta:"label=Enable Host Terminal;type=boolean;keywords=host,terminal,shell,console,nsenter,privileged,root,ssh;category=security;description=Allow users with the system:host-terminal permission to open a root shell on the Docker host through a privileged helper container. Disabled by default because it grants full root access to the host machine"`
	HostTerminalImage               SettingVariable `key:"hostTerminalImage,envOverride" meta:"label=Host Terminal Helper Image;type=text;keywords=host,terminal,shell,image,tools,nsenter,helper;category=security;description=Override the image used for the host terminal helper container. Leave empty to use the Arcane tools image. Must contain nsenter"`
