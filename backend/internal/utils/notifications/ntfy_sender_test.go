	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(url string) bool {
				return url == "ntfy://ntfy.sh/test-topic"
			},
		},
		{
			...
			wantErr: false,
			check: func(url string) bool {
				return url == "ntfy://ntfy.example.com/alerts"
			},
		},
		{
			...
			wantErr: false,
			check: func(url string) bool {
				return url == "ntfy://ntfy.example.com:8080/updates"
			},
		},
		{
			...
			wantErr: false,
			check: func(url string) bool {
				return url == "ntfy://user:pass@ntfy.example.com:443/private"
			},
		},
		{
			...
			wantErr: false,
			check: func(url string) bool {
				return url == "ntfy://ntfy.sh/alerts?priority=high&tags=warning%2Cserver"
			},
		},
		{
			name: "missing topic",
