...
func TestRusticServiceRunUsesArcaneToolsImageInternal(t *testing.T) {
	t.Parallel()
	...
	output, err := NewRusticService(nil).runInternal(
		context.Background(),
		dockerClient,
		rusticRepositoryInternal{environment: []string{"RUSTIC_REPOSITORY=/repository"}},
		"secret",
		[]string{"snapshots", "--json"},
	)
	require.NoError(t, err)
	require.Equal(t, "snapshot-json", output)
	require.Equal(t, volumehelper.DefaultToolsImage, created.Image)
	require.Equal(t, []string{"rustic"}, created.Entrypoint)
