...
func (s *VolumeService) ListDirectory(ctx context.Context, volumeName, dirPath string) ([]volumetypes.FileEntry, error) {
	...
	cmd := []string{"sh", "-c", fmt.Sprintf("stat -c '%%n|%%s|%%Y|%%f|%%A' %s/* %s/.[!.]* 2>/dev/null || true", quotedPath, quotedPath)}
...
func (s *VolumeService) DownloadFile(ctx context.Context, volumeName, filePath string) (io.ReadCloser, int64, error) {
	...
	targetPath := path.Join("/volume", sanitizedPath)
	cmd := []string{"stat", "-c", "%s", targetPath}
	stdout, _, err := s.execInContainerInternal(ctx, containerID, cmd)
	if err != nil {
		cleanup()
...
func (s *VolumeService) CreateBackup(ctx context.Context, volumeName string, user models.User) (*models.VolumeBackup, error) {
	...
	backupID := fmt.Sprintf("%s-%d", volumeName, time.Now().Unix())
...
func (s *VolumeService) RestoreBackup(ctx context.Context, volumeName, backupID string, user models.User) error {
	...
	config := &container.Config{
		...
		Cmd: []string{
			"sh",
			"-c",
			fmt.Sprintf("tmp=/volume/.restore_tmp_$$; rm -rf \"$tmp\"; mkdir -p \"$tmp\"; tar -xzf /backups/%s -C \"$tmp\"; rm -rf /volume/* /volume/.[!.]* /volume/..?* 2>/dev/null || true; mv \"$tmp\"/* /volume/ 2>/dev/null || true; mv \"$tmp\"/.[!.]* /volume/ 2>/dev/null || true; mv \"$tmp\"/..?* /volume/ 2>/dev/null || true; rmdir \"$tmp\" 2>/dev/null || true", filename),
		},
		Labels: map[string]string{
			libarcane.InternalContainerLabel: "true",
		},
...
func (s *VolumeService) UploadAndRestore(ctx context.Context, volumeName string, archive io.Reader, filename string, user models.User) error {
	...
	err = dockerClient.CopyToContainer(ctx, containerID, tmpDir, archive, container.CopyToContainerOptions{})
	if err != nil {
	...
	moveCmd := fmt.Sprintf("mv %s/* /volume/ 2>/dev/null || true; mv %s/.[!.]* /volume/ 2>/dev/null || true; mv %s/..?* /volume/ 2>/dev/null || true; rmdir %s 2>/dev/null || true", tmpDir, tmpDir, tmpDir, tmpDir)
