...
func (s *ProjectService) ApplyGitSyncProjectFiles(ctx context.Context, projectID string, composeContent string, gitEnvContent *string, user models.User) (*models.Project, error) {
	...
	effectiveEnvContent, err := s.resolveEffectiveEnvContentForGitSyncUpdateInternal(proj.Path, gitEnvContent)
	if err != nil {
	...
	if err := s.persistGitSyncEnvFilesInternal(proj.Path, projectsDirectory, gitEnvContent); err != nil {
...
func (s *ProjectService) resolveEffectiveEnvContentForGitSyncUpdateInternal(projectPath string, gitEnvContent *string) (*string, error) {
	state, err := projects.ReadProjectEnvState(projectPath)
	if err != nil {
		return nil, fmt.Errorf("read project env state: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("build effective env content: %w", err)
	}
...
func (s *ProjectService) persistGitSyncEnvFilesInternal(projectPath, projectsDirectory string, gitEnvContent *string) error {
	state, err := projects.ReadProjectEnvState(projectPath)
	if err != nil {
		return fmt.Errorf("read project env state: %w", err)
	}
	...
	if err := fs.WriteProjectFile(projectsDirectory, projectPath, projects.GitSourceEnvFileName, *gitEnvContent); err != nil {
		return err
	}
	if strings.TrimSpace(overrideContent) == "" {
		if err := fs.RemoveProjectFile(projectsDirectory, projectPath, projects.OverrideEnvFileName); err != nil {
