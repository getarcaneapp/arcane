...
test.describe("Projects Page", () => {
  ...
  test("should show project actions menu", async ({ page }) => {
    ...
    const firstProject = realProjects.find((p) => p.name === firstProjectName) ?? realProjects[0];
    const runningCount = Number(firstProject?.runningCount ?? 0);
    const serviceCount = Number(firstProject?.serviceCount ?? 0);
    ...
    if (serviceCount > 0) {
      await expect(downItem).toBeVisible();
    } else {
      await expect(downItem).toHaveCount(0);
    }
    ...
    if (runningCount > 0) {
      ...
      await expect(restartItem).toHaveCount(0);
    }
