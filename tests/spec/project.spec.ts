...
async function createProjectViaUI(page: Page, projectName: string) {
  ...
  const envFile = TEST_ENV_FILE.replace(
    /CONTAINER_NAME=.*/m,
...
test.describe("Projects Page", () => {
  test("should display the main heading and description", async ({ page }) => {
    ...
    await expect(
      page.getByText("View and Manage Compose Projects"),
    ).toBeVisible();
  });
  ...
  test("should show project actions menu", async ({ page }) => {
    test.skip(
      !realProjects.length,
      "No projects available for actions menu test",
    );
    ...
    const hasStateAction =
      (await upItem.count()) > 0 ||
      (await downItem.count()) > 0 ||
      (await restartItem.count()) > 0;
    expect(hasStateAction).toBe(true);
    await expect(
      page.getByRole("menuitem", { name: "Pull & Redeploy" }),
    ).toBeVisible();
    await expect(page.getByRole("menuitem", { name: "Destroy" })).toBeVisible();
  });

  test("should navigate to project details when project name is clicked", async ({
    page,
  }) => {
    test.skip(
      !realProjects.length,
      "No projects available for navigation test",
    );
  ...
  test("should allow searching/filtering projects", async ({ page }) => {
    ...
    if (firstProject?.name) {
      ...
      await expect(
        page.getByRole("link", { name: firstProject.name }),
      ).toBeVisible();
  ...
  test("should display project status badges", async ({ page }) => {
    test.skip(
      !realProjects.length,
      "No projects available for status badge test",
    );
...
test.describe("New Compose Project Page", () => {
  ...
  test("should display the create project form", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "My New Project" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Docker Compose File" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Environment (.env)" }),
    ).toBeVisible();
  });
  ...
  test("should validate required fields", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "My New Project" }).click();
    await page
      .getByRole("textbox", { name: "My New Project" })
      .fill("test-project");
    await page.getByRole("textbox", { name: "My New Project" }).press("Enter");
  });

  test("should enable Create Project after entering a valid name", async ({
    page,
  }) => {
    ...
    const stateUnsafe = observedErrors.filter((e) =>
  ...
  test("should create a new project successfully", async ({ page }) => {
    ...
    const emptyServicesState = page.getByText(
      /No services found for this project/i,
    );
  ...
  test("should send selected deploy split-button options in the up request", async ({
    page,
  }) => {
    ...
    try {
      ...
      await page
        ...
        .filter({ hasText: "Always" })
        .first()
        .click();

      await deployMenuTrigger.click();
      await page
        .getByRole("menuitemcheckbox", { name: /Force recreate containers/i })
        .click();
  ...
  test("should destroy the project and remove files from disk", async ({
    page,
  }) => {
...
test.describe("GitOps Managed Project", () => {
  test("should show read-only alert when project is GitOps managed", async ({
    page,
  }) => {
  ...
  test("should display Sync from Git button when GitOps managed", async ({
    page,
  }) => {
    const gitOpsProject = realProjects.find((p) => p.gitOpsManagedBy);
    test.skip(!gitOpsProject, "No GitOps-managed projects found");
    ...
    await expect(
      page.getByRole("button", { name: "Sync from Git" }),
    ).toBeVisible();
  });

  test("should show last sync commit when GitOps managed", async ({ page }) => {
    const gitOpsProject = realProjects.find(
      (p) => p.gitOpsManagedBy && p.lastSyncCommit,
    );
    test.skip(
      !gitOpsProject,
      "No GitOps-managed projects with sync commit found",
    );
  ...
  test("should have compose editor in read-only mode when GitOps managed", async ({
    page,
  }) => {
    const gitOpsProject = realProjects.find((p) => p.gitOpsManagedBy);
    test.skip(!gitOpsProject, "No GitOps-managed projects found");
  ...
  test("should have env editor in read-only mode when GitOps managed", async ({
    page,
  }) => {
    const gitOpsProject = realProjects.find((p) => p.gitOpsManagedBy);
    test.skip(!gitOpsProject, "No GitOps-managed projects found");
  ...
  test("should allow editing for non-GitOps managed projects", async ({
    page,
  }) => {
    const regularProject = realProjects.find(
      (p) => !p.gitOpsManagedBy && p.status === "stopped",
    );
    test.skip(
      !regularProject,
      "No regular (non-GitOps) stopped projects found",
    );
...
test.describe("Project Detail Page", () => {
  test("should display project details for existing project", async ({
    page,
  }) => {
    test.skip(
      !realProjects.length,
      "No projects available for detail page test",
    );
    ...
    await expect(
      page.getByRole("button", { name: firstProject.name, exact: false }),
    ).toBeVisible();

    await expect(page.getByRole("tab", { name: /Services/i })).toBeVisible();
    await expect(
      page.getByRole("tab", { name: /Configuration|Config/i }),
    ).toBeVisible();
  ...
  test("should display services tab content", async ({ page }) => {
    ...
    const projectWithServices =
      realProjects.find((p) => p.serviceCount > 0) || realProjects[0];
    await page.goto(
      `/projects/${projectWithServices.id || projectWithServices.name}`,
    );
    await page.waitForLoadState("networkidle");
    ...
    if ((await nginxService.count()) > 0) {
      ...
      const anyServiceBadge = page
        .locator("text=/running|stopped|unknown/i")
        .first();
  ...
  test("should display configuration editors", async ({ page }) => {
    test.skip(
      !realProjects.length,
      "No projects available for configuration test",
    );
    ...
    await expect(
      page.getByRole("heading", { name: "compose.yaml" }),
    ).toBeVisible();
    ...
    if (isTreeView) {
      const composeFileButton = page
        .getByRole("button", { name: "compose.yaml" })
        .first();
  ...
  test("should show logs tab for running projects", async ({ page }) => {
    ...
    if (logsSelected === "true") {
      ...
      await expect(
        page.getByRole("button", { name: /^(Start|Stop)$/i }),
      ).toBeVisible();
      await expect(
