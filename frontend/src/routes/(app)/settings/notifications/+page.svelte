<script lang="ts">
	...
	import { EmailProviderForm, DiscordProviderForm, TelegramProviderForm, SignalProviderForm, SlackProviderForm, NtfyProviderForm, AppriseProviderForm } from './providers';
	...
	const hasChanges = $derived(emailHasChanges || discordHasChanges || telegramHasChanges || signalHasChanges || slackHasChanges || ntfyHasChanges || appriseHasChanges);
	...
	async function onSubmit() {
		...
		if (!(emailValid && discordValid && telegramValid && signalValid && slackValid && ntfyValid)) {
		...
		<fieldset disabled={isReadOnly} class="relative">
			...
			<Tabs.Root bind:value={notificationsTab}>
				...
				<Tabs.Content value="apprise" class="mt-4 space-y-4 sm:mt-6 sm:space-y-6">
					...
					<AppriseProviderForm
						bind:values={appriseValues}
						disabled={isReadOnly}
						{isTesting}
						onTest={testAppriseNotification}
					/>
				</Tabs.Content>
			</Tabs.Root>
		</fieldset>
