<script lang="ts">
	...
	let {
		...
		/** popover-sized list heights instead of the full sheet heights */
		compact?: boolean;
	} = $props();
...
<div class={cn('flex-1 overflow-y-auto', compact ? 'max-h-[min(60vh,480px)]' : 'min-h-[68vh]')}>
	...
	{#if activityStore.loading && activityStore.activities.length === 0}
		<div class={cn('flex items-center justify-center p-6 text-center', compact ? 'min-h-48' : 'min-h-96')}>
			<div class="space-y-2">
