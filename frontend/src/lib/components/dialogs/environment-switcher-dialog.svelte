<script lang="ts">
	...
	$effect(() => {
		if (open && !hasLoaded) {
			...
			currentPage = 1;
			hasLoaded = true;
			loadEnvironments(1, '', false);
		} else if (!open) {
			// Reset hasLoaded when dialog closes so it loads fresh next time
			hasLoaded = false;
		}
	});

	async function loadEnvironments(page: number = 1, search: string = '', append: boolean = false) {
		if (page === 1) {
			isLoading = true;
		} else {
			isLoadingMore = true;
	...
	const debouncedSearch = debounced((query: string) => {
		currentPage = 1;
		loadEnvironments(1, query, false);
	}, 300);
	...
	function handleScroll(e: Event) {
		...
		if (scrollHeight - scrollTop - clientHeight < 50) {
			if (!isLoadingMore && currentPage < totalPages) {
				loadEnvironments(currentPage + 1, searchQuery, true);
			}
		}
	}
