	...
	<Sheet.Content class="p-6">
		...
		<form onsubmit={preventDefault(handleSubmit)} class="grid gap-4 py-4">
			...
			<FormInput
				...
				description="Git username (optional for public repos)"
				bind:input={$inputs.username}
			/>
			<FormInput
				label="Personal Access Token"
				type="password"
				placeholder={isEditMode ? 'Leave blank to keep current token' : 'ghp_xxxxxxxxxxxxx'}
				description="Git personal access token or password"
			...
			<SwitchWithLabel
				id="autoSyncSwitch"
				label="Auto Sync"
				description="Automatically sync and deploy changes from the repository"
